    ...
    If($VMachines){
        ...
        ForEach($VMachine in $VMachines){
            ...
            $VMData.Add([PSCustomObject]@{ 
                ...
                'Charge Capacity GB' = ($VMTotalHDChargeCapacity / [double]1GB)     # Column I
                'Host'               = $VMachine.VMHost.Name                        # Column J
                'Cluster'            = $VMachine.VMHost.Parent.Name                 # Column K
                'CPU Cost'           = $CPUCurrency                                 # Column L
                'RAM Cost'           = $RAMCurrency                                 # Column M
                'Storage'            = $StorageCurrency                             # Column N
                'Flat Rate'          = $FlatCurrency                                # Column O
                'Monthly Cost'       = $MonthlyCurrency                             # Column P
            }) | Out-Null
        }
    }
    #endregion
...
If($VMDataLastRow -gt 1){
    ...
    $VMChargeCapacityGBColumn = "'VMs'!`$I`$2:`$I`$$VMDataLastRow"
    $VMCPUCostColumn          = "'VMs'!`$L`$2:`$L`$$VMDataLastRow"
    $VMRAMCostColumn          = "'VMs'!`$M`$2:`$M`$$VMDataLastRow"
    $VMStorageCostColumn      = "'VMs'!`$N`$2:`$N`$$VMDataLastRow"
    $VMFlatCostColumn         = "'VMs'!`$O`$2:`$O`$$VMDataLastRow"
    $VMMonthlyCostColumn      = "'VMs'!`$P`$2:`$P`$$VMDataLastRow"
    ...
    $VMDataStyle += New-ExcelStyle -Range $VMChargeCapacityGBColumn -NumberFormat '0.00'
